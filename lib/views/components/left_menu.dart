...
import 'package:amplify/views/components/theme_toggle_switch.dart';
...
class LeftMenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return isFOrMobile
        ? Drawer(
            child: _mainDrawerView(context, isFOrMobile),
  ...
  Widget _buildListTile(BuildContext context, MenuData menuData) {
    ...
    return selectedMenu == menuData.menuItem
        ? Container(
  ...
  _mainDrawerView(BuildContext context, bool isMobile) {
    return Column(
      children: [
        Expanded(
          child: ListView(
  ...
  _itemListTile(BuildContext context, MenuData menuData) {
    final theme = Theme.of(context);
      final bool isDarkMode = theme.brightness == Brightness.dark;
    return ListTile(
      leading: Padding(
        ...
        child: SvgPicture.asset(
            selectedMenu == menuData.menuItem
                ...
                : menuData.iconPath,
            colorFilter:
                ColorFilter.mode((selectedMenu == menuData.menuItem && !isDarkMode)? AppTheme.primaryColor:theme.iconTheme.color!, BlendMode.srcIn)),
        ...
        child: Text(
          ...
          style: theme.textTheme.bodyLarge?.copyWith(
              fontWeight: selectedMenu == menuData.menuItem
                  ? FontWeight.bold
                  : FontWeight.normal,
              color:  (selectedMenu == menuData.menuItem && !isDarkMode)? const Color(0xFF08548A):const Color(0xFFC9E7F5)),
        ),
      ),
      onTap: () {
